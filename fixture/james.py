...
class JamesHelper:
    ...
    def ensure_user_exists(self, username, password):
        ...
        james_telnet_session = JamesHelper.Session(james_config['host'], james_config['port'],
                                                   james_config['username'], james_config['password'])

        if james_telnet_session.is_user_registered(username):
            james_telnet_session.reset_password(username, password)
        else:
            james_telnet_session.create_user(username, password)

        james_telnet_session.quit()
    ...
    class Session:
        ...
        def is_user_registered(self, username):
            ...
            # return 'True' if the 1-st list's element was found
            # (so, the user exists)
            return res[0] == 0
